...
type HomeType = 'single' | 'condo' | 'townhouse' | 'apartment';
type Room = 'kitchen' | 'bath' | 'bedroom' | 'living' | 'garage' | 'yard' | 'attic' | 'basement';

const TYPES: Array<{ key: HomeType; title: string; meta: string }> = [
  { key: 'single',    title: 'Single-family',  meta: 'Detached, own lot' },
...
const ROOMS: Array<{ key: Room; title: string }> = [
  ...
  { key: 'yard',     title: 'Yard / Exterior' },
  { key: 'attic',    title: 'Attic' },
  { key: 'basement', title: 'Basement' },
];
...
export default function HomeEdit() {
  ...
  const [type, setType] = useState<HomeType>('single');
  const [yearBuilt, setYearBuilt] = useState(1998);
  const [rooms, setRooms] = useState<Set<Room>>(new Set(['kitchen', 'bath', 'living', 'attic']));

  const toggleRoom = (r: Room) => {
  ...
  return (
    <NoirScreen>
      ...
      >
        ...
        <AmberCTA label="Save home profile" variant="primary" size="lg" onPress={onSave} />
