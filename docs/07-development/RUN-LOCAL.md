...
Проходит без ошибок — значит TS strict не сломан. Если правил `components/ui/*` или `constants/tokens.ts` — обязательно прогони перед коммитом.

### 9. Что НЕ поддерживается в текущем билде

- **Нативная камера** — все точки входа (`/your-house`, FAB) ведут в статичные мок-экраны. Expo Camera не интегрирован.
- **Supabase / Claude API** — вся data из `mock/repair.ts`
- **Adapty / Paywall** — экран Paywall из Sanctuary v1 удалён, Noir v2 paywall не проектировался
...
- `app/` — маршруты expo-router (filename-based)
- `components/ui/` — 14 дизайн-примитивов (`NoirScreen`, `NoirCard`, `AmberCTA`, …)
- `constants/tokens.ts` — единственный источник цветов/радиусов/шрифтов/gradient-ов
- `mock/repair.ts` — моки для экранов
- `hooks/useAppFonts.ts` — загрузка Google Fonts
