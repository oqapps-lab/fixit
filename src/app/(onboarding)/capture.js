...
export default function Capture() {
    ...
    const continueWithAsset = async (asset) => {
        if (asset === undefined || asset === null || typeof asset.uri !== 'string') {
            return;
        }
    ...
    return (<NoirScreen glow="none">
      ...
      <View style={styles.viewfinder}>
        <Svg width="100%" height="100%" viewBox="0 0 420 800" preserveAspectRatio="xMidYMid slice">
          ...
          <Path d="M170 330 h80 v40 h-24 v60 h8 c20 0 30 14 30 30 h-108 c0-16 10-30 30-30 h8 v-60 h-24 z" fill="rgba(255,255,255,0.08)" stroke="rgba(255,255,255,0.14)" strokeWidth={1}/>
      ...
      <View style={[styles.bottomBand, { paddingBottom: insets.bottom + spacing.xl }]}>
        <View style={styles.bottomRow}>
          <Pressable onPress={onPickFromGallery} hitSlop={8} accessibilityRole="button" accessibilityLabel="Use saved photo" style={styles.smallBtn}>
            <Text allowFontScaling={false} style={styles.smallBtnText}>GALLERY</Text>
          </Pressable>

          <Pressable onPress={onCapturePhoto} hitSlop={8} accessibilityRole="button" accessibilityLabel="Capture" style={styles.shutterOuter}>
            <View style={styles.shutterInner}>
              <CameraGlyph size={24} color={colors.text}/>
            </View>
          </Pressable>

          <Pressable onPress={onPickFromGallery} hitSlop={8} accessibilityRole="button" accessibilityLabel="Describe with text" style={styles.smallBtn}>
