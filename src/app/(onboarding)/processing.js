...
import { getEstimateDraft, setEstimateError, setEstimateResult } from '@/lib/estimate/draft';
import { runEstimatePipeline } from '@/lib/estimate/pipeline';
const STAGES = [
    { label: 'Identify the issue', answer: 'Issue detected', tone: 'cyan' },
    { label: 'Check twelve retailers', tone: 'amber' },
    { label: 'Calculate local labor rates', tone: 'cyan' },
...
export default function Processing() {
    ...
    useEffect(() => {
        ...
        const stageTimer = setInterval(() => {
            setStageIdx((current) => (current < STAGES.length - 1 ? current + 1 : current));
        }, 1250);
        ...
        const runPipeline = async () => {
            try {
                ...
                setEstimateResult({
                    ...
                    estimateRecordId: result.estimateRecordId,
                    requiresSubscription: result.requiresSubscription,
                });
                router.replace('/your-house');
            }
            catch (error) {
      ...
      <View style={[
        ...
        <View style={styles.stages}>
          {STAGES.map((s, i) => {
            ...
            return (<NoirCard key={s.label} variant={state === 'active' ? 'elevated' : 'default'} radius="md" padding={14} style={[styles.stageRow, state === 'pending' ? { opacity: 0.45 } : null]}>
                <View style={styles.stageInner}>
                  <View style={styles.marker}>
                    {state === 'done' ? (<CheckGlyph size={22} color={colors.mint}/>) : state === 'active' ? (<View style={styles.activeDot}/>) : (<View style={styles.pendingDot}/>)}
                  </View>
                  ...
                  {state === 'done' && s.answer ? (<DocRef tone="mint">{s.answer}</DocRef>) : null}
                </View>
              </NoirCard>);
        })}
        </View>
