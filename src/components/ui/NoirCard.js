 ...
 */
export function NoirCard({ variant = 'default', radius = 'md', padding, style, children, ...rest }) {
    const br = RADIUS_MAP[radius];
    const isGlass = variant === 'glass' || variant === 'default' || variant === 'elevated';
    const fillAlpha = variant === 'elevated' ? 0.09 :
        variant === 'glass' ? 0.05 :
            variant === 'blueprint' ? 0.04 :
                0.07;
    const borderColor = variant === 'outlined' ? colors.hairlineStrong :
    ...
    return (<View style={[containerStyle, style]} {...rest}>
      ...
      {isGlass && Platform.OS === 'ios' ? (<BlurView pointerEvents="none" intensity={variant === 'elevated' ? 40 : 28} tint="dark" style={StyleSheet.absoluteFill}/>) : null}

      {/* translucent tint on top of blur */}
      {isGlass ? (<View pointerEvents="none" style={[
      ...
      {variant === 'blueprint' ? (<View pointerEvents="none" style={[
                StyleSheet.absoluteFill,
                { backgroundColor: colors.surfaceBlueprint },
            ]}/>) : null}

      {/* top-edge highlight — catches "light" */}
      {isGlass ? (<View pointerEvents="none" style={styles.edgeHighlight}/>) : null}
