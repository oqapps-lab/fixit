...
import { getFunctionConfig } from '../_shared/config.js';
...
function safeDomainFromSource(source) {
...
function normalizeTrustedSources(value) {
  ...
  return value.flatMap((item) => {
    ...
    return [{ ...provisionalSource, domain: resolvedDomain }];
  });
}
...
async function resolveAuthenticatedUserId(adminClient, request) {
...
async function callAnthropic({
  ...
  const firstResponse = await fetch(anthropicEndpoint, {
    method: 'POST',
    headers,
  ...
  const secondResponse = await fetch(anthropicEndpoint, {
    method: 'POST',
    headers,
...
function parseAnalysisText(assistantText, trustedSources) {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const anthropicResponse = await callAnthropic({
      ...
      anthropicEndpoint: config.anthropicEndpoint,
